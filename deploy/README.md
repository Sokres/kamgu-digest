...
**Для заказчика (пошаговая установка с нуля):** [INSTALL.md](INSTALL.md) — сервер, PostgreSQL, `.env`, запуск API и сайта.

Ниже — расширенный чеклист для продакшена: ОС, Docker, секреты, прокси, CI/CD и устранение неполадок.
