...
- **Модель периодов:** ключ снимка — `YYYY-MM-DD` (режим `day`, по умолчанию для ручных запусков и daily/weekly cron) или `YYYY-MM` (режим `month`, для monthly/quarter cron). Повторный запуск в **тот же** период обновляет снимок (upsert). Сравнение в `/trends` — между соседними сохранёнными периодами. Поле `period_mode` в теле `POST /digests/periodic`: `"day"` | `"month"`.
