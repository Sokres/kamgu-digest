...
- **Модель периодов:** ключ снимка — `YYYY-MM` (UTC). Повторный запуск в том же месяце **обновляет** существующий снимок (upsert), а не создаёт новую точку тренда. Частота cron (ежедневно, еженедельно, ежемесячно) задаёт только **когда** пересчитывается дайджест; гранулярность периода всегда **месячная**. Сравнение и выводы в `/trends` появляются при **двух и более** разных периодах (например `2026-06` и `2026-07`). Второй период можно получить дождавшись следующего месяца или указав `force_period: "2026-07"` в теле `POST /digests/periodic`.
